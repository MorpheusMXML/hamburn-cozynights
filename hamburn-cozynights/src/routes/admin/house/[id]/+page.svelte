					...
					{#if roomKindEntry(room.kind) || readFeatures(room.features, 'room').length > 0 || room.bedMix}
						<div class="room-details">
							{#if roomKindEntry(room.kind)}
								<span class="chip">{roomKindEntry(room.kind)?.icon} {roomKindEntry(room.kind)?.label}</span>
							{/if}
							{#each readFeatures(room.features, 'room') as feature}
								<span class="chip">{featureEntry(feature)?.icon} {featureEntry(feature)?.label}</span>
							{/each}

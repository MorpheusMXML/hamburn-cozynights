    ...
    <aside class="info-column" in:fly={{ x: -20, duration: 500, delay: 200 }}>
        <div class="status-card turquoise">
            ...
            <div class="big-number">
                {beds.filter(b => b.enabled !== false && b.occupied).length} <span class="divider">/</span> {beds.filter(b => b.enabled !== false).length}
            </div>
            <p class="label">CLAIMED SPOTS</p>
            <div class="capacity-info">
                Total Capacity: {beds.length} ({(beds.filter(b => b.enabled !== false).length / (beds.length || 1) * 100).toFixed(0)}% ACTIVE)
            </div>
        </div>
    ...
    <main class="beds-column" in:fade={{ delay: 400 }}>
        ...
        <div class="beds-grid">
            {#each beds as bed (bed.id)}
                <div class="bed-card" class:occupied={bed.occupied} class:disabled={isBookingActive} class:inactive={bed.enabled === false} in:fade>
                    ...
                    <div class="bed-actions">
                        <form action="?/toggleEnabled" method="POST" use:enhance={() => handleAction(bed.id, 'toggle')}>
                            ...
                            <button class="btn-icon" class:orange={bed.enabled === false} class:disabled={isBookingActive} disabled={isBookingActive} title={bed.enabled === false ? 'Activate' : 'Deactivate'}>
                        ...
                        <form action="?/toggleOccupied" method="POST" use:enhance={() => handleAction(bed.id, 'toggle')}>
                            ...
                            <button class="btn-icon turquoise" title="Toggle occupancy" disabled={isBookingActive || bed.enabled === false} class:disabled={isBookingActive || bed.enabled === false}>🔄</button>
                        </form>

                        {#if isVerified}
                            <form action="?/deleteBed" method="POST" use:enhance={() => handleAction(bed.id, 'delete')}>
                                <input type="hidden" name="id" value={bed.id} />
                                <button class="btn-icon vanish" title="Delete spot" disabled={isBookingActive} class:disabled={isBookingActive}>🗑</button>
                            </form>
                        {/if}
                    </div>
                </div>
...
<style>
  ...
  .capacity-info { margin-top: 1rem; font-size: 0.6rem; color: #444; font-weight: 900; letter-spacing: 1px; }
  ...
  .form-panel.disabled { opacity: 0.4; filter: grayscale(1); pointer-events: none; }
  .lockdown-notice { position: absolute; top: 1.5rem; right: 2rem; color: #fb923c; font-size: 0.65rem; font-weight: 900; letter-spacing: 1px; }
  ...
  .bed-card.disabled { opacity: 0.5; filter: grayscale(0.8); }
  .bed-card.inactive { border-style: dashed; background: #0a0a0a; border-color: #333; }
  .bed-card.occupied { border-color: #311; background: #150a0a; }
  .bed-card:hover:not(.disabled) { transform: translateY(-3px); border-color: #444; }
  .bed-card.occupied:hover:not(.disabled) { border-color: #ef4444; }
  ...
  .btn-icon.disabled { opacity: 0.3; cursor: not-allowed !important; }
  .btn-icon:hover:not(.disabled) { color: #fff; transform: scale(1.1); }
  .btn-icon.turquoise:hover:not(.disabled) { border-color: #2dd4bf; color: #2dd4bf; box-shadow: 0 0 10px rgba(45, 212, 191, 0.2); }
  .btn-icon.orange:hover:not(.disabled) { border-color: #fb923c; color: #fb923c; box-shadow: 0 0 10px rgba(251, 146, 60, 0.2); }
  .btn-icon.vanish:hover:not(.disabled) { border-color: #f87171; color: #f87171; background: #211; box-shadow: 0 0 10px rgba(248, 113, 113, 0.2); }

    ...
    <div class="title-container">
      ...
      <h1 class="burning-laser-title">
        {#each titleLetters as { char, color, delay, offset }, i}
          <span 
            class="letter" 
            style="--color: {color}; --delay: {delay}ms; --offset: {offset}px"
          >
            {char === " " ? "\u00A0" : char}
          </span>
        {/each}
              ...
              {#if isHovering}
                  <div class="party-zone">
                      {#each Array(40) as _, i}
                          <div class="particle" style="--angle: {Math.random() * 360}deg; --dist: {60 + Math.random() * 120}px; --color: {neonColors[i % neonColors.length]}; --delay: {Math.random() * 1.5}s; --size: {0.8 + Math.random() * 1.5}rem">
                              {['✨', '🔥', '🌵', '⚡️', '🌈', '💎', '🚀', '🎡'][i % 8]}
                          </div>
                      {/each}
                  </div>
...
<style>
  ...
  .title-container { position: relative; margin-bottom: 5rem; padding: 2rem; }
  ...
  @keyframes letter-ignite {
    0% { opacity: 0; transform: translateY(var(--offset)) scale(2); filter: blur(20px); }
    70% { opacity: 1; transform: translateY(-5px) scale(0.9); filter: blur(0); }
    100% { 
        opacity: 1; 
        transform: translateY(0) scale(1); 
        filter: blur(0); 
        color: #fff; 
        text-shadow: 0 0 10px rgba(255,255,255,0.8), 0 0 30px var(--color); 
    }
  }
  ...
  @keyframes shoot-out {
      ...
      15% { opacity: 1; }
      100% { 
          transform: translate(calc(cos(var(--angle)) * var(--dist)), calc(sin(var(--angle)) * var(--dist))) rotate(720deg) scale(2); 
          opacity: 0; 
      }
  }
  ...
  @media (max-width: 600px) {
    .burning-laser-title { font-size: 2.8rem; letter-spacing: -2px; }
    .input-group { flex-direction: column; border-radius: 32px; padding: 15px; gap: 15px; }
    button { padding: 1.2rem; width: 100%; }
    input { padding: 1rem; text-align: center; }
    .party-zone { display: none; }
  }
</style>
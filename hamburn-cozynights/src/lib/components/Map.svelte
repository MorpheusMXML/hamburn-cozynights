<script lang="ts">
  ...
  $: labelPositions = (houses || []).reduce((acc, house) => {
      const isSomeoneBelow = houses.some(other => 
          other.id !== house.id && 
...
<div class="map-wrapper" on:mousemove={handleMouseMoveGlobal} role="presentation">
  ...
  >
    ...
    <!-- Dynamic Mouse Glow -->
    <circle cx={mouseX} cy={mouseY} r={isEditorMode ? 150 : 100} fill="url(#mouseGlow)" pointer-events="none" opacity={isEditorMode ? 1 : 0.5} />
...
<style>
  .map-wrapper { width: 100%; height: 100%; background: #000; position: relative; cursor: crosshair; }
  svg { width: 100%; height: 100%; display: block; }
  
  .map-image { transition: filter 1s ease; }

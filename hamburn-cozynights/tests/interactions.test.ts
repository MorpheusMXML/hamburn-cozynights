...
describe('Interactions & Registration', () => {
    ...
    it('should fail login with an invalid booking code', async () => {
        ...
        const request = { formData: async () => formData } as any;
        const result = await loginActions.login({ request, cookies: mockCookies, locals: mockLocals } as any) as any;
...
describe('Room Load & Booking Logic', () => {
    ...
    it('should allow booking an available bed', async () => {
        ...
        expect(mockAdminPb.update).toHaveBeenCalledWith('bed1', expect.objectContaining({ occupied: true, order: 'order1' }));
    });

    it('should allow unbooking a bed', async () => {
        mockPb.getOne.mockResolvedValueOnce({ is_booking_active: true }); // Settings
        mockAdminPb.getFirstListItem.mockResolvedValueOnce({ id: 'order1' }); // Order lookup
        mockAdminPb.getFullList.mockResolvedValueOnce([{ id: 'bed1', occupied: true, order: 'order1' }]); // Current beds
...
describe('Admin Management Actions', () => {
    ...
    it('should create a room and bed templates in staging mode', async () => {
        ...
        await houseAdminActions.createRoom({ request, params: { id: 'house1' }, locals: mockLocals } as any);
        
        expect(mockPb.create).toHaveBeenCalledWith(expect.objectContaining({ name: 'Villa Suite', house: 'house1' }));
        expect(mockPb.create).toHaveBeenCalledWith(expect.objectContaining({ label: 'Spot 1', room: 'room1' }));
    });
    ...
    it('should create a bed if staging mode (booking inactive)', async () => {
        ...
        expect(mockPb.create).toHaveBeenCalledWith(expect.objectContaining({ label: 'New Bed', room: 'room1' }));
    });

# 🗄️ Data Model & Location Templates
...
data lives and how it is backed up: [DEPLOYMENT.md, section 8](DEPLOYMENT.md#8-backups-and-where-data-lives).
...
write" means an approved `admins` record (see [SECURITY.md](SECURITY.md)).
...
- **While booking is live**, the structure is locked on the server: houses
  ...
  deleted, and templates can't be imported ([WORKFLOW.md](WORKFLOW.md)).
...
- `order_number` is still stored in plain text next to `order_hash`, for
  legacy imports (see [SECURITY.md](SECURITY.md)).

...
- **Encrypted settings.** PocketBase's own settings (among them the SMTP password for booking e-mails) are stored encrypted with a per-environment key (`PB_ENCRYPTION_KEY`, PocketBase's `--encryptionEnv`), so a database backup alone doesn't give them away. The Google client secret of the admin sign-in is stored by PocketBase outside that encryption, in the `admins` collection's options: backups are therefore treated as secret material (root-only snapshots, an encrypted restic repository), and a leaked backup means rotating that client secret. Details: [Environments & deployment](../develop/deployment#pocketbase-settings-key).

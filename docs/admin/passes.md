...
| *The check-in could not be saved: the booking system is not reachable right now. Try again.* · *We could not reach the server. Check the connection and try again.* | The app can't reach its database, or your phone has no connection. The second message also shows when your admin session ended while the page was open (for example because the weekly Google sign-in is due). On the pass page the same trouble reads *That did not work: the server could not be reached, or your admin session has ended.* | Try again in a moment; if the connection is fine, reload the page and sign in again. Scanning again is safe: if the first try went through, the pass reads ALREADY CHECKED IN. The pass page needs the database too (*The booking system is not reachable right now. Please try again in a minute.*), so meanwhile go by the guest's screenshot. |

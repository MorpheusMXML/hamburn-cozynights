...
Choose whatever is at hand; all four show the same result.
...
| **Camera on the check page** | <kbd>📷 Scan with camera</kbd> reads the QR code with the phone's or laptop's camera, in any current browser over HTTPS. It tries the rear camera first and falls back to the front one; a recognised code is checked right away, without pressing anything, and the camera closes: press <kbd>📷 Scan with camera</kbd> again for the next guest. <kbd>Close camera</kbd> stops it. The first time, the browser asks whether this site may use the camera. |
...
![Check booking passes: the code field, the camera button and the latest checks, one valid and one unknown](../assets/screenshots/admin-check.webp)

### What the result means

| Result | Meaning | What to do |
| --- | --- | --- |
| ✅ **VALID** | The ticket holds a spot: shown with house, room, spot, burner name, the name from the ticket list and its e-mail (shortened). | Welcome them. **open room** jumps to the room in the admin area. |
...
<p align="center"><img src="../assets/screenshots/admin-pass-check.webp" alt="A pass opened with the phone camera by a signed-in admin: the check result on top of the pass" width="300" /></p>
...
| *The booking system is not reachable right now. Try again.* · *We could not reach the server. Check the connection and try again.* | The app can't reach its database, or your phone has no connection. The second message also shows when your admin session ended while the page was open (for example because the weekly Google sign-in is due). | Try again in a moment; if the connection is fine, reload the page and sign in again. The pass page needs the database too (*The booking system is not reachable right now. Please try again in a minute.*), so meanwhile go by the guest's screenshot. |
...
| *This isn't a CozyNights booking pass. Check the code or the link.* on a pass page | The link doesn't end in a pass code, for example because it was cut off when copied. | Open the link from the confirmation again, or type the code on **Check passes**. |

## Privacy and security

- **The pass code is not the ticket code.** The ticket code signs in and can change or release bookings; the pass code can only *show* a booking. It is random (12 characters without the look-alikes 0, O, 1, I and L), unrelated to the ticket code and can't be guessed.
...
- **What the crew sees:** the check result with the name from the ticket list and the shortened address, only when signed in to the admin area.

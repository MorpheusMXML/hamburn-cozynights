...
- **The pass page** shows house, room, spot and burner name, the QR code and the code. When the crew wrote them down in the [camp layout](./camp-layout), it also shows the bed — the kind of bed, and for a bunk bed which level it is and where the other one is, e.g. *Upper bunk · above B1* — and the features at the spot, e.g. *🔥 Heated · 🔌 Power socket* (the spot's own together with its room's and house's). Nothing is guessed: a detail nobody filled in is simply left out. <kbd>Save QR code</kbd> stores the QR code as an image named `cozynights-pass-<code>.gif` (on a phone it goes to the photos); a screenshot works just as well. <kbd>Camp map</kbd> leads back to the map. A link typed without dashes or in lower case works too. If the ticket holds no spot right now, the page says so instead: *This ticket holds no spot right now. Pick one on the map while booking is open.*
...
- **It shows what the pass page shows:** house, room, spot, burner name, the pass code, the event and its dates — and, when the crew wrote them down, the bed (*Upper bunk · above B1*) and the features at the spot (*🔥 Heated · 🔌 Power socket*): on the back of the Apple pass, as *Your bed* on the Google pass. Never the ticket code, the name on the ticket, the e-mail address or the check-in.

...
- [ ] **Switch back to staging** (superuser: ⚡ Switch right now → 🛠 Staging). Leave *Don't notify the guests* ticked and choose <kbd>Switch & release …</kbd>; the timer is paused and **every booking is released**, spots marked 🔄 TAKEN included (spots free, burner names forgotten, ticket codes stay). The confirmation says *guests not notified*. With the requests gone, the spots the crew booked for special-needs requests are ordinary bookings now and go too, so <kbd>🧨 Clear all bookings</kbd> next to the switch stays greyed out: nothing is left to clear.

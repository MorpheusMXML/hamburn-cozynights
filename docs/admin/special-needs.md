...
- who **decided** and when, and the **spot** the ticket holds right now.

| Button | What happens |
| --- | --- |
| <kbd>Approve</kbd> | The request is approved; the guest gets a message that the crew is picking a spot. |
...
| <kbd>Release spot</kbd> | The spot becomes free again (a special-needs spot stays one); the request stays approved, so book another one. |
| <kbd>Decline</kbd> | The guest gets a message that the crew can't offer a special-needs spot, and books like everyone else once booking opens. To decline an approved request that already has a spot, release the spot first. |
...
| Approved, no spot yet | **Your special-needs request was approved** — the crew picks a spot |
...
- **A guest can change a request while it waits**, and **withdraw it at any time**. Withdrawing deletes it; a spot the crew already booked stays booked.
- **A spot the crew booked is fixed.** The guest can rename it, but can't move or release it themselves: they ask the crew. You move or release it here.
- **Clear all bookings keeps these spots**, with their burner names. The dialog says how many.
...
- **Only admins can read it**, here and nowhere else. It is stored encrypted, so the PocketBase dashboard and backups only hold unreadable text.

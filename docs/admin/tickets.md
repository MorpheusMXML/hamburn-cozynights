...
| E-mail | If the ticket holds a spot, the new address gets its own message: *A CozyNights spot came with your ticket*, with the spot and the new pass. The old address gets nothing. |
...
- **The review** lists new tickets, changed tickets (old → new address or name), problem rows, addresses shared by several tickets, tickets that are not in the file, and unchanged ones. Under *Not in the file* you can tick tickets the shop no longer lists (cancelled, or from another file): a ticked ticket is deleted with the import, together with its Telegram link and its special-needs request. Only tickets without a spot can be ticked — free the spot first if one is meant to go. Nothing there is preselected. Everything new and changed starts ticked; <kbd>All</kbd> ticks or clears a whole group. Small groups start unfolded, big ones folded; with more than twelve changes, the field *Filter by code, e-mail or name* narrows every group down to what you are looking for.
...
- **Nothing is deleted unless you tick it.** Tickets that aren't in the file stay as they are; the review lists them with shortened codes under *Not in the file*, and only the ones you tick there go. On the server, cancelled tickets can also be removed with `cozy-admin.sh tickets remove`. Either way only tickets that hold no spot.
...
Operators can load the same CSV on the server with `cozy-admin.sh tickets import roster.csv` (`--dry-run` first). It checks the same rules, but refuses the whole file when one row is broken and has no per-row choice.

A **changed** address is where the two differ. On the server a ticket that still carries something of its holder — a booking pass, a Telegram chat, a special-needs request, a burner name or a check-in — is refused with its line and what it carries, and nothing is imported; a ticket's *first* address always goes through, because nobody is displaced by it. Either load such a file here, where 🔁 **New holder** decides per ticket, or repeat the server import with `--hand-over`, which treats **every** changed address in the file as a new holder: the old pass, burner name, Telegram chat, special-needs request and check-in are dropped, the spot stays, and the new address gets the *passed on to you* e-mail. `--dry-run --hand-over` lists what it would do first. Either way the crew group gets the 📥 import message. See [Ticket codes](./event-checklist#ticket-codes).
:::

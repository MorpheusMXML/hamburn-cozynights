...
| Unit | `npm test` | – | The logic in isolation, PocketBase mocked: admin sign-in flow and session handling, role checks, booking rules, encryption, rate limit, special-needs requests (with the in-memory stand-in `tests/fake-pb.ts`), and every guest message text for each mix of spot change and request news (`tests/notify-messages.test.ts`), including the message texts admins change (`tests/message-texts.test.ts`), the live numbers of the control center with their snapshot cache, ETag and poll rules (`tests/live-stats.test.ts`) and what the Intel panel makes of them — Berlin-time buckets, filters, the house table, the attention list (`tests/intel.test.ts`), how a refused form points at its fields (`tests/field-alert.test.ts`), and the guest session: dead codes, the round stamp, an unreachable PocketBase (`tests/guest-session.test.ts`). Fast (< 1 s) — run it all the time. |
...
- **Guest sessions** – the migration's `guest_round` field, the round a sign-in carries, and the two things that end a session: the ticket leaves the list, or a reset starts a new round (both delete the cookies; the rules themselves are in `tests/guest-session.test.ts`).
...
- **Check-in** – only approved admins and superusers can write a check-in (guests and pending requests are refused), it goes when the booking goes and moves along with a crew move, a hand-over resets it, and it sends the guest nothing. The pass check at the gate names a ticket without a name by its masked code, never by the code itself.
...
- **Tickets & template review** – a ticket handed over gets a new pass and its own hand-over e-mail to the new address (none to the old one), the old holder's Telegram and burner name are gone, the old pass is dead; a corrected address keeps pass and Telegram; the search finds codes in any case (also hash-only ones) and addresses with `+`/`_` exactly; the ticket list import creates and updates only the chosen tickets; applying chosen template changes moves, renames, adds and removes only those, keeps the other bookings, makes a backup and tells the released guest.
- **Wallet passes** – the download routes hand out a signed Apple pass and write the Google object, and note both for the sync; Apple's web service registers a device, says which passes changed since a tag, hands out the current pass (304 while it is unchanged) and unregisters it again, each with that pass's own token; the sync pushes a move to the registered devices and to Google, voids the pass of a ticket that changed hands, and `forget-contacts` deletes the device registrations. Apple's push service and Google's API are stand-ins in the test process.
- **Notifications** – confirmation e-mails (booked, one "changed" for a move, released by the guest or with a deleted room), no mail for tickets without an address, a newly imported address, retries and the crew alert after the last one (the ticket by its masked code, no holder name, no full address); Telegram link, `/stop`, blocked bot, unknown links; crew alerts for access changes, phase switches with the admin's name, an outage of Telegram, and a real OAuth2 sign-in through the admin guard; the pass as a picture with its buttons, `/pass` in the chat, the guest commands in the bot's menu, and the Telegram and wallet offers in the e-mail until the guest has connected; `cozy-admin tickets import` (dry run, broken file, stdin) and `notify status` / `test`.
...
`scripts/test-stack.sh` starts `docker-compose.test.yml`: PocketBase with an in-memory database (always empty at start) and, for the smoke tests, the app image. Two stand-ins replace the outside world: **Mailpit** catches every e-mail (web UI on port 8293) and `tests/fixtures/mock-services.mjs` plays the Telegram Bot API, Google's OAuth2 endpoints and the Google Wallet API (port 8292). The stack also gets a throwaway certificate chain and service-account key for the wallet passes, so the smoke tests download a real (test-signed) pass; nothing ever reaches Apple or Google. Credentials are random per run — the service account, the app's `ENCRYPTION_KEY` and PocketBase's settings key `PB_ENCRYPTION_KEY` — and thrown away with the stack. The stack lifts the per-minute cap on guest e-mails (`COZY_MAILS_PER_MINUTE`), so the suites' messages aren't held back. It uses its own compose project (`cozynights-verify`) and ports (8290 / 3290 / 8292 / 8293), so it can't collide with a dev or staging stack on the same machine.

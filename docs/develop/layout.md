...
8. **Fixed UI text gets room at every width.** Below the width where a header's parts fit next to each other, the least important part goes first (the map drops the house counter, then the link texts, then words of the phase badge) or the parts get rows of their own (the admin menu became a sidebar from 1100 px and a ☰ drawer below; on small phones the top bar drops the ADMIN badge before the ⚡️ and Eject collide). Squeezed next to the account, the admin pages once stacked up in a column. The test checks every 40 px, so a breakpoint that is off shows up.

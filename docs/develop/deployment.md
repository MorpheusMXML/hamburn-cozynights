...
- **Not covered: the Google OAuth client secret.** PocketBase stores per-collection OAuth2 providers in the `admins` collection's options, which `--encryptionEnv` does not touch (checked in the 0.40.4 source and in the database). A database backup still contains it, so backups stay secret material: deploy snapshots are root-only on the server, the restic repository is encrypted, PocketBase's own ZIPs live inside the volume. A leaked backup means rotating the Google client secret (Cloud Console, then `.env`, then recreate the PocketBase container).
